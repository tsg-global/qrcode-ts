import QR8bitByte from "qrcode/qr_8bit_byte";
import QRBitBuffer from "qrcode/qr_bit_buffer";
import QRPolynomial from "qrcode/qr_polynomial";
import QRRSBlock from "qrcode/qr_rs_block";
import QRUtil from "qrcode/qr_util";
...
export default class QRCodeModel {
	...
	private modules: Array<Array<boolean>>;
	private errorCorrectLevel: number;
	private dataCache: Array<number>;
	...
	public getBestMaskPattern() {
		let minLostPoint = 0;
		let pattern = 0;
		for (var i = 0; i < 8; i++) { this.makeImpl(true, i);
			...
			while (true) {
				for (var c = 0; c < 2; c++) {
					if (this.modules[row][col - c] == null) {
						...
						bitIndex--;
						if (bitIndex == -1) { byteIndex++;
							bitIndex = 7; }
					}
				}
				row += inc;
				if (row < 0 || this.moduleCount <= row) { row -= inc;
					inc = -inc;
					break; }
			}
		}
